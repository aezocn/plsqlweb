...
function WebFXLoadTree(sText, sXmlSrc, oAction, sBehavior, sIcon, sOpenIcon, sOnblur, sOnfocus, sOnmouseover, sOnmouseout, Sonclick) { //后加sOnblur, sOnfocus
	WebFXTree.call(this, sText, oAction, sBehavior, sIcon, sOpenIcon, sOnblur, sOnfocus, sOnmouseover, sOnmouseout, Sonclick);
...
WebFXLoadTree.createLoadingItem = function () {
	return new WebFXTreeItem(webFXTreeConfig.loadingText, null, null,
							 webFXTreeConfig.loadingIcon, null);
};
...
function WebFXLoadTreeItem(sText, sXmlSrc, oAction, eParent, sIcon, sOpenIcon, sOnblur, sOnfocus, sOnmouseover, sOnmouseout, Sonclick) {
	WebFXTreeItem.call(this, sText, oAction, eParent, sIcon, sOpenIcon, sOnblur, sOnfocus, sOnmouseover, sOnmouseout, Sonclick);
...
WebFXLoadTree._attrs = ["text", "src", "action", "id", "target", "onblur", "onfocus", "onmouseover", "onmouseout", "onclick"]; //后加
...
WebFXLoadTree.createItemFromElement = function (oNode) {
	...
	var jsNode = new WebFXLoadTreeItem(jsAttrs.html || "", jsAttrs.src, action,
									   null, jsAttrs.icon, jsAttrs.openIcon, jsAttrs.onblur, jsAttrs.onfocus, jsAttrs.onmouseover, jsAttrs.onmouseout, jsAttrs.onclick); 
	if (jsAttrs.text) {
		jsNode.setText(jsAttrs.text);
	}
	//后加onblur
	if (jsAttrs.onblur) {
		jsNode.onblur = jsAttrs.onblur;
	}
	//后加onfocus
	if (jsAttrs.onfocus) {
		jsNode.onfocus = jsAttrs.onfocus;
	}
	//后加onmouseover
	if (jsAttrs.onmouseover) {
		jsNode.onmouseover = jsAttrs.onmouseover;
	}
	//后加onmouseout
	if (jsAttrs.onmouseout) {
		jsNode.onmouseout = jsAttrs.onmouseout;
	}
	//后加onclick
	if (jsAttrs.onclick) {
		jsNode.onclick = jsAttrs.onclick;
	}
